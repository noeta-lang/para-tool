...
- **A consuming manifest needs the scope (array) form to reach a submodule.** With `para = { path = "…", package = "para/tool" }`, `use para.tool` resolves and `use para.tool.boundary` is `` module `para.tool` has no export `boundary` ``; `para = [{ path = "…", package = "para/tool" }]` is what works. A single-module sibling never hits this, which is why its README shows the table form. Both example manifests here use the array form even though each names one package.
